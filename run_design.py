...
target_dir = "CRISPR/20260916_crRNA_FLASHit"
...
# Extract only the ARO accession number (e.g., >3002070) to clean FASTA headers
...
# Fixed functional components for in vitro transcription (IVT) oligo synthesis
t7_promoter = "TAATACGACTCACTATAG"      # 18 nt T7 promoter
crrna_handle = "GTTTTAGAGCTATGCTGTTTTG" # 22 nt crRNA scaffold/repeat handle
...
for block in content:
    ...
    for i in range(seq_len - 23):
        if seq[i:i+3] == "TTT": # Cas12a/Cas9 PAM motif (e.g., TTTN / TTTV)
            sp = seq[i+4:i+24]  # Extract 20 nt spacer immediately adjacent to PAM
            if len(sp) == 20 and "N" not in sp and sp not in seen_spacers:
                seen_spacers.add(sp)
                start_pos = i + 5  # 1-based start coordinate of the 20 nt spacer
    ...
    for i in range(seq_len - 23):
        if rc_seq[i:i+3] == "TTT":
            ...
            if len(sp) == 20 and "N" not in sp and sp not in seen_spacers:
                seen_spacers.add(sp)
                # Map coordinates back to the original (+) strand reference sequence
